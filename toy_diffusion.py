...
attn_edge_type[adj != 0] = edge_attr
...
node_atom = torch.ones([1, n_nodes, 6])
node_charge = torch.ones([1, n_nodes, 1])
pos = torch.rand([1, n_nodes, 3])



transformer = TransformerEncoder(num_atoms = 5,
                                    num_charges = 8,
                                    num_in_degree = 40,
                                    num_out_degree = 40,
                                    num_edges = 300,
                                    ...
                                    neighbor_combine_embedding='cat',
                                    ...
                                    equivariant_in_attention_dropout = 0.0,
                                    equivariant_use_x_layernorm = True,
                                    equivariant_use_dx_layernorm = True,
...
pos = torch.cat([torch.rand([1, n_nodes, 3]), torch.rand([1, n_nodes, 3])+1], dim=0)


# x_out, adj_out, pos_out = transformer(node_atom.repeat(2, 1, 1), attn_bias.unsqueeze(0).repeat(2, 1, 1), spatial_pos.unsqueeze(0).repeat(2, 1, 1), edge_input.unsqueeze(0).repeat(2, 1, 1, 1, 1).float(), attn_edge_type.unsqueeze(0).repeat(2, 1, 1, 1).float(), pos, adj.unsqueeze(0).repeat(2, 1, 1), node_mask.repeat(2, 1, 1), edge_mask.repeat(2, 1, 1, 1))

model = Transformer_dynamics(transformer, context_node_dim=0, n_dims=3, condition_time=True)

model(torch.tensor([0.7, 0.5]), torch.cat([pos, node_atom.repeat(2, 1, 1)], dim=-1), attn_bias.unsqueeze(0).repeat(2, 1, 1), spatial_pos.unsqueeze(0).repeat(2, 1, 1), edge_input.unsqueeze(0).repeat(2, 1, 1, 1, 1).float(), attn_edge_type.unsqueeze(0).repeat(2, 1, 1, 1).float(), adj.unsqueeze(0).repeat(2, 1, 1), node_mask.repeat(2, 1, 1), edge_mask.repeat(2, 1, 1, 1))