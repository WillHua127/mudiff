...
def train_epoch(args, loader, epoch, model, model_dp, model_ema, ema, device, dtype, property_norms, optim,
    ...
    for i, data in enumerate(loader):
        ...
        #spatial_positions = data['spatial_positions'].to(device)
        #edge_input = data['edge_input'].to(device)
        ...
        if i % args.n_report_steps == 0:
            print(f"\rEpoch: {epoch}, iter: {i}/{n_iterations}, "
                  f"Loss {loss.item():.3f}, NLL: {nll.item():.3f}, "
                  f"RegTerm: {reg_term.item():.1f}, "
                  f"GradNorm: {grad_norm:.1f}")

            print(f"\rLoss_xh: {loss_dict['loss_t'].mean(0).item():.3f}, Loss_adj: {loss_dict['loss_adj_t'].mean(0).item():.3f}, "
                  f"KL_prior_xh: {loss_dict['kl_prior'].mean(0).item():.3f}, KL_prior_adj: {loss_dict['kl_prior_adj'].mean(0).item():.3f}")

        nll_epoch.append(nll.item())
        '''
        if ((epoch+1) % args.test_epochs == 0) and (i % args.visualize_every_batch == 0) and not (epoch == 0 and i == 0):
            start = time.time()
...
def save_and_sample_chain(model, args, device, dataset_info, prop_dist,
    ...
    one_hot, charges, x, adj = sample_chain(args=args, device=device, flow=model,
                                       n_tries=1, dataset_info=dataset_info, prop_dist=prop_dist)

    vis.save_xyz_file(f'outputs/{args.exp_name}/epoch_{epoch}_{batch_id}/chain/',
                      one_hot, charges, x, adj, dataset_info, id_from, name='chain')
...
def sample_different_sizes_and_save(model, nodes_dist, args, device, dataset_info, prop_dist,
    ...
    for counter in range(int(n_samples/batch_size)):
        ...
        one_hot, charges, x, adj, node_mask = sample(args, device, model, prop_dist=prop_dist,
                                                nodesxsample=nodesxsample,
                                                dataset_info=dataset_info)
        #print(f"Generated molecule: Positions {x[:-1, :, :]}")
        vis.save_xyz_file(f'outputs/{args.exp_name}/epoch_{epoch}_{batch_id}/', one_hot, charges, x, adj, dataset_info,
...
def analyze_and_save(epoch, model_sample, nodes_dist, args, device, dataset_info, prop_dist,
    ...
    molecules = {'one_hot': [], 'x': [], 'adj': [], 'node_mask': []}
    for i in range(int(n_samples/batch_size)):
        nodesxsample = nodes_dist.sample(batch_size)
        one_hot, charges, x, adj, node_mask = sample(args, device, model_sample, dataset_info, prop_dist,
                                                nodesxsample=nodesxsample)
