...
from qm9.data.prepare.process import process_xyz_files, process_xyz_gdb9, process_qdb9_qm9, get_split_data
...
def download_dataset_qm9(datadir, dataname, splits=None, calculate_thermo=True, exclude=True, cleanup=False):
    ...
    file_path = download_url(raw_url, gdb9dir)
    extract_zip(file_path, gdb9dir)
    os.unlink(file_path)

    file_path2 = download_url(raw_url2, gdb9dir)
    os.rename(join(gdb9dir, '3195404'), join(gdb9dir, 'uncharacterized.txt'))
...
#     gdb9_tar_data = join(gdb9dir, 'dsgdb9nsd.xyz.tar.bz2')
    ...
    if splits is None:
        splits = gen_splits_gdb9(gdb9dir, cleanup=False)

    print('Start Processing...')
    # Process GDB9 dataset, and return dictionary of splits
    all_data = process_qdb9_qm9(join(gdb9dir, 'gdb9.sdf'), join(gdb9dir, 'gdb9.sdf.csv'), stack=True)
#     if os.path.exists(join(gdb9dir, 'qm9_mols.pt')):
#         all_data = torch.load(join(gdb9dir, 'qm9_mols.pt'))
#     else:
#         all_data = process_qdb9_qm9(join(gdb9dir, 'gdb9.sdf'), join(gdb9dir, 'gdb9.sdf.csv'), stack=True)
#         torch.save(all_data, join(gdb9dir, 'qm9_mols.pt'))
...
#             gdb9_tar_data, process_xyz_gdb9, file_idx_list=split_idx, stack=True)
    ...
    gdb9_data = get_split_data(all_data, splits)
    ...
    if calculate_thermo:
        ...
        therm_energy = get_thermo_dict(gdb9dir, cleanup=False)
