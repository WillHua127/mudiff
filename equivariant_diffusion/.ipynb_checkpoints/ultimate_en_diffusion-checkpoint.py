...
import math
import torch
from egnn import ultimate_models
...
class EnVariationalDiffusion(torch.nn.Module):
    ...
    def __init__(
            self,
            dynamics: models.Transformer_dynamics, in_node_nf: int, n_dims: int, max_nodes: int = 29,
            timesteps: int = 1000, parametrization='eps', noise_schedule='learned',
            noise_precision=1e-8, loss_type='vlb', norm_values=(1., 1., 1.),
            norm_biases=(None, 0., 0.), transition_type='marginal', adj_type='digress', include_charges=True):
        super().__init__()
        ...
        if self.adj_type == 'digress':
            self.extra_features = ExtraFeatures(extra_features_type='all', max_n_nodes=max_nodes)
            self.BCE = torch.nn.BCEWithLogitsLoss()
            self.Edim_output = 2
            if transition_type == 'uniform':
                self.transition_model = transition.DiscreteUniformTransition(e_classes=1)
                e_limit = torch.ones(1) / 1
                self.limit_dist = e_limit
    ...
    def kl_prior_adjacency(self, E, edge_mask):
        ...
        limit_dist_E, probE = diffusion_utils.mask_distributions(true_E=E, pred_E=probE, edge_mask=edge_mask)
    ...
    def log_pE_given_z0_without_constants(self, h, E, node_mask, edge_mask, gamma_0, z_t, t, context):
        # Compute noise values for t = 0.
        bs, n_nodes = node_mask.shape
        sigma_0 = self.sigma(gamma_0, target_tensor=edge_mask)
        Q0 = self.transition_model.get_Qt(beta_t=sigma_0, device=E.device)

        probE0 = E @ Q0  # (bs, n, n, de_out)

        E0 = diffusion_utils.sample_discrete_features(probE=probE0, edge_mask=edge_mask)

        E0 = F.one_hot(E0, num_classes=self.Edim_output).float()
        assert (E.shape == E0.shape)

        E0 = E0 * edge_mask.unsqueeze(-1)
        extra_h, extra_e, extra_y = self.extra_features(E0, node_mask)
        net_out, probE0 = self.phi(z_t, t, node_mask.view(bs, n_nodes, 1).float(), edge_mask.view(bs, n_nodes, n_nodes, 1).float(), context, h=h, y=extra_y, adjs=E0)
    ...
    def compute_adjacency_loss(self, pred, true, bs, n_nodes, alphas):
        ...
        adj_loss = torch.stack([self.BCE(p_adj[mask, :], t_adj[mask, :]) for (p_adj,t_adj,mask) in zip(pred_adj, true_adj, mask_adj)])
    ...
    def compute_loss(self, x, h, node_mask, edge_mask, context, t0_always):
        ...
        if self.adj_type == 'dppm':
            ...
            adjs = torch.ones_like(edge_mask)
            adjs = adjs.view(bs, n_nodes, n_nodes)
            adjs = adjs * edge_mask.view(bs, n_nodes, n_nodes)
            noise_adjs_t, grad_log_noise_t = diffusion_utils.generate_noise_adjs(adjs, edge_mask, beta_t, bs, n_nodes, device=node_mask.device)
            
        elif self.adj_type == 'digress':
            Qtb = self.transition_model.get_Qt_bar(alpha_t, device=x.device)  #(bs, de_in, de_out)
            bs, n_nodes, dims = x.shape
            node_mask_adj = node_mask.squeeze().bool()
            edge_mask_adj = edge_mask.view(bs, n_nodes, n_nodes).bool()
            
            a = torch.bernoulli(torch.empty(bs, n_nodes, n_nodes).uniform_(0, 1))
            adjs = torch.cat([a, 1-a])
            adjs = adjs.view(bs, n_nodes, n_nodes, -1) * edge_mask.view(bs, n_nodes, n_nodes, 1)
            prob_adjs = adjs @ Qtb.unsqueeze(1)
            sampled_adjs = diffusion_utils.sample_discrete_features(prob_adjs, edge_mask_adj)
            E_t = F.one_hot(sampled_adjs, num_classes=self.Edim_output)
            E_t = E_t * edge_mask.view(bs, n_nodes, n_nodes, 1)
            
            extra_h, extra_e, extra_y = self.extra_features(E_t, node_mask_adj)
            y_2d = extra_y
            h_2d = h['categorical']
        ...
        if self.adj_type == 'digress':
            net_out, pred_adj = self.phi(z_t, t, node_mask, edge_mask, context, h=h_2d, y=y_2d, adjs=E_t)
            adj_bce = self.compute_adjacency_loss(pred_adj, adjs, bs, n_nodes, alpha_t)
            kl_prior_adj = self.kl_prior_adjacency(adjs, edge_mask_adj)
        else:
            net_out, pred_adj = self.phi(z_t, t, node_mask, edge_mask, context)
        ...
        if t0_always:
            ...
            if self.adj_type == 'digress':
                loss_adj_term_0 = -self.log_pE_given_z0_without_constants(h_2d, adjs, node_mask_adj, edge_mask_adj, gamma_t, z_t, t, context)
                loss_adj_t = loss_adj_term_0 * t_is_zero.squeeze() + t_is_not_zero.squeeze() * adj_bce
