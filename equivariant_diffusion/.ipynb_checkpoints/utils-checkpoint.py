...
def get_spatial_positions(edge_attr, edge_mask, device):

    spatial_positions = []
    
    for (adj, mask) in zip(edge_attr, edge_mask):
        mask = mask.numpy()
        shortest_path_result, _ = algos.floyd_warshall(adj.sum(-1).long().numpy())
    ...
    return spatial_positions.long().to(device)
