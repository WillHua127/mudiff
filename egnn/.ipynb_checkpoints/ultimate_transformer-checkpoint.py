...
from egnn.ultimate_embedding import *
...
class TransformerMEncoder(nn.Module):
    ...
    def __init__(
        ...
        num_equivariant_encoder_layers: int = 6,
        ...
        self.geom_info = GeometricInformation(
                                n_layers=num_equivariant_encoder_layers,
                                num_edges=num_edges,
                                num_kernel=num_3d_bias_kernel,
                                cutoff_lower=cutoff_lower,
                                cutoff_upper=cutoff_upper,
    ...
    def forward(
        self,
        node_features, node_charges, attn_bias, spatial_pos=None, edge_input=None, edge_type=None, pos=None, adj=None, node_mask=None, edge_mask=None,
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:        
        bs, n_nodes = node_features.size()[:2]
        

        ### Preparing padding mask
        padding_mask = (node_features[:,:,0]==0) # B x T x 1
        ...
        x = self.atom_feature(node_features, node_charges, adj=adj)
        
        
        ### A.2 Obtaining (2d) attention biases
        attn_bias = self.molecule_attn_bias(node_features, node_charges, attn_bias, spatial_pos, edge_input, edge_type)

        
        ### A.3 Obtaining (3d) attention biases & Encoding (with 3d information)
        delta_pos = None
        if (self.molecule_3d_bias is not None) and not (pos == 0).all():
            attn_bias_3d, merged_edge_features, delta_pos = self.molecule_3d_bias(node_features, node_charges, pos)
        ...
        if (self.molecule_2d_neighbor_embedding is not None) and (adj is not None):
            x_neighbors_2d = self.molecule_2d_neighbor_embedding(node_features, node_charges, adj, edge_type)

            
        ### A.5 Encoding (with 3d neighborhood information)
        x_neighbors_3d = None
        if (self.molecule_3d_neighbor_embedding is not None) and not (pos == 0).all():
            x_neighbors_3d = self.molecule_3d_neighbor_embedding(node_features, node_charges, pos, adj)
        ...
        if (self.edge_feature is not None) and (adj is not None):
            encoded_adj = self.edge_feature(node_features, node_charges, adj, edge_type)
