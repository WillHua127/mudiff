...
class AtomEmbedding(nn.Module):
    ...
    def __init__(self, num_atoms, num_charges, num_in_degree, num_out_degree, hidden_dim, n_layers, use_2d=True):
        ...
        # 1 for graph token
        self.atom_encoder = nn.Linear(num_atoms, hidden_dim, bias=False)
        self.charger_encoder = nn.Embedding(num_charges + 1, hidden_dim, padding_idx=0)
    ...
    def forward(self, node_features, node_charges, adj=None):
        bs, n_nodes = node_features.size()[:2]

        node_charges = node_charges.squeeze()
        
        # node feauture + graph token
        node_atom_feature = self.atom_encoder(node_features) # [n_graph, n_node, n_hidden]
        node_charge_feature = self.charger_encoder(node_charges) # [n_graph, n_node, n_hidden]
        node_feature = node_atom_feature + node_charge_feature
...
class EdgeEmbedding(nn.Module):
    ...
    def __init__(self, num_atoms, num_charges, num_edges, hidden_dim, edge_hidden_dim, n_layers):
    ...
    def forward(self, node_features, node_charges, adj, attn_edge_type):
        bs, n_nodes = node_features.size()[:2]

        # symmetric [n_graph, n_node, n_node, unknown]
        node_charges = node_charges.squeeze()
        ...
        edge_feature = self.edge_encoder(attn_edge_type).mean(-2)
...
class ExtraGraphFeatureEmebedding(nn.Module):
    ...
    def __init__(self, num_atoms, max_weight, atom_weights, extra_feature_type, graph_hidden_dim, n_layers, use_2d):
        ...
        self.extra_features = ExtraFeatures(extra_features_type=extra_feature_type, max_n_nodes=num_atoms)  if self.use_2d else None
    ...
    def forward(self, node_features, adj=None, edge_type=None, node_mask=None):
        ...
        spatial_graph_embedding = self.extra_features(edge_type, node_mask.squeeze().bool()) if (self.extra_features is not None) and (adj is not None) else None
                
        molecular_graph_embedding = self.extra_molecular_features(node_features) if (self.extra_molecular_features is not None) else None
...
class MoleculeAttnBias(nn.Module):
    def __init__(self, num_heads, num_atoms, num_edges, num_spatial, num_edge_dis, hidden_dim, edge_type, multi_hop_max_dist, n_layers, use_2d):
    ...
    def forward(self, node_features, node_charges, attn_bias, spatial_pos, edge_input, attn_edge_type):
...
class MoleculeNeighbor2DEmbedding(torch_geometric.nn.MessagePassing):
    def __init__(self, num_atoms, num_charges, num_edges, n_layers, hidden_dim):
    ...
    def forward(self, node_features, node_charges, adj, edge_feature):
        ...
        edge_weight = self.edge_encoder(edge_feature).mean(-2) * adj.unsqueeze(-1)
...
class Molecule3DBias(nn.Module):
    ...
    def forward(self, node_features, node_charges, pos):
...
class MoleculeNeighbor3DEmbedding(torch_geometric.nn.MessagePassing):
    def __init__(self, num_atoms, num_charges, num_edges, n_layers, num_kernel, hidden_dim, cutoff_lower=0.0, cutoff_upper=5.0, distance_projection='exp', trainable=True, use_2d=False):
    ...
    def forward(self, node_features, node_charges, pos, adj=None):
        ...
        if self.use_2d and (adj is not None):
            cutoffs = torch.where(((adj>0) + cutoffs) < 1, cutoffs, torch.tensor(1.))
        ...
        edge_weight = self.nonlin_proj(edge_feature) * cutoffs.unsqueeze(-1)
